class BinarySearchTreeMap:
    ...
    def recur_subtree_insert(self, curr_root, key, value):
        if curr_root.item.key < key:
            if curr_root.right == None:
                ...
                recur_subtree_insert(self, curr_root.right, key, value)
        elif curr_root.item.key > key:
            if curr_root.left == None:
                item = BinarySearchTreeMap.Item(key, value)
                new_node = BinarySearchTreeMap.Node(item)
                curr_root.right = BinarySearchTreeMap.Node(item)
                new_node.parent = curr_root
            else:
                recur_subtree_insert(self, curr_root.left, key, value)
        else:
            item = BinarySearchTreeMap.Item(key, value)
            curr_root.Item == item
    def insert(self, key, value):
        self.recur_subtree_insert(self.root, key, value)

    def fca(bst, node_a, node_b):
        akey = node_a.item.key
        bkey = node_b.item.key
        rkey = bst.root.item.key
        if akey < rkey and bkey < rkey:
            new_tree = BinarySearchTreeMap()
            new_tree.root = bst.root.left 
            return fca(new_tree, node_a, node_b)
        elif akey > rkey and bkey > rkey:
            new_tree = BinarySearchTreeMap()
            new_tree.root = bst.root.right
            return fca(new_tree, node_a, node_b)
        else:
            return bst.root
